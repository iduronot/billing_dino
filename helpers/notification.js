...
// ─────────────────────────────────────────
// NOTIFIKASI TIKET KOMPLAIN
// ─────────────────────────────────────────

/**
 * Kirim notifikasi Telegram ke teknisi tertentu
 */
 ...
 */
async function notifyTicketCreated(pool, ticket, techIds = []) {
    try {
        const s = await getSettings(pool, ['telegram_bot_token', 'telegram_chat_id', 'company_name']);
        const company = s.company_name || 'ISP';

        // Pesan ke grup admin (Telegram)
        const adminMsg = `🎫 <b>TIKET BARU #${ticket.id}</b>

` +
            `📋 ${ticket.title}
` +
            `👤 Pelanggan: ${ticket.customer_name || (ticket.source === 'infrastruktur' ? '📡 Infrastruktur' : 'Umum')}
` +
            `🏷 Kategori: ${ticket.category || '-'}
` +
            `⚡ Prioritas: ${ticket.priority?.toUpperCase()}
` +
            (ticket.location ? `📍 Lokasi: ${ticket.location}
` : '') +
            `
${company}`;

        await sendTelegram(pool, adminMsg);

        // Notifikasi ke setiap teknisi yang ditugaskan
        if (techIds.length > 0) {
            const [techs] = await pool.query(
                'SELECT username, telegram_id, phone FROM users WHERE id IN (?)', [techIds]);
            for (const tech of techs) {
                const techMsg = `🔔 <b>Tiket Baru Ditugaskan</b>
...
` +
                    `⚡ Prioritas: ${ticket.priority?.toUpperCase()}
` +
                    `👤 ${ticket.customer_name || (ticket.source === 'infrastruktur' ? 'Infrastruktur' : 'Umum')}
` +
...
Segera tangani!`;
                // Telegram ke teknisi
                if (tech.telegram_id) await sendTelegramToUser(pool, tech.telegram_id, techMsg);
                // WA ke teknisi
                if (tech.phone) await sendWhatsApp(pool, tech.phone, techMsg.replace(/<b>|<\/b>/g, '*'));
            }
        }
    } catch(e) { console.error('[NOTIF] ticketCreated error:', e.message); }
}

/**
 * Notifikasi saat tiket di-assign ke teknisi baru
 */
async function notifyTicketAssigned(pool, ticket, techId) {
    try {
        const [[tech]] = await pool.query('SELECT username, telegram_id, phone FROM users WHERE id=?', [techId]);
        if (!tech) return;

        const msg = `👷 <b>Penugasan Tiket</b>

` +
            `Halo ${tech.username}!

` +
            `Tiket berikut ditugaskan kepada Anda:
` +
            `🎫 #${ticket.id} - ${ticket.title}
` +
            `⚡ Prioritas: ${ticket.priority?.toUpperCase()}
` +
            `👤 ${ticket.customer_name || 'Umum'}
` +
            (ticket.location ? `📍 ${ticket.location}
` : '') +
            `
Segera tangani!`;
        ...
        if (tech.telegram_id) await sendTelegramToUser(pool, tech.telegram_id, msg);
        if (tech.phone) await sendWhatsApp(pool, tech.phone, msg.replace(/<b>|<\/b>/g, '*'));
    } catch(e) { console.error('[NOTIF] ticketAssigned error:', e.message); }
}

/**
 * Notifikasi saat tiket selesai
 */
async function notifyTicketClosed(pool, ticket) {
    try {
        const s = await getSettings(pool, ['telegram_bot_token', 'telegram_chat_id']);
        const adminMsg = `✅ <b>TIKET SELESAI #${ticket.id}</b>
...
` +
            `👤 ${ticket.customer_name || 'Umum'}
` +
            (ticket.response_note ? `📝 Resolusi: ${ticket.response_note}
` : '');
        await sendTelegram(pool, adminMsg);
    } catch(e) { console.error('[NOTIF] ticketClosed error:', e.message); }
}
