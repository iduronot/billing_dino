...
router.get('/', async (req, res) => {
    try {
        ...
        if (dateTo)   { conditions.push(`DATE(i.${safeField}) <= ?`); params.push(dateTo); }
        ...
        const [[{ total }]] = await pool.query(
            `SELECT COUNT(*) as total FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id ${whereClause}`, params
        );

        // Stats global — 1 query gabungan (lebih cepat)
        const [[globalStats]] = await pool.query(`
            SELECT
                SUM(status='paid')   as paid,
                SUM(status='unpaid') as unpaid,
                SUM(status='unpaid' AND due_date < CURDATE()) as overdue,
                COALESCE(SUM(CASE WHEN status='paid' AND MONTH(paid_at)=MONTH(NOW()) AND YEAR(paid_at)=YEAR(NOW()) THEN amount ELSE 0 END),0) as totalRevenue
            FROM invoices`);
        const { paid, unpaid, overdue, totalRevenue } = globalStats;

        // Stats dalam range filter (jika ada)
        let filteredRevenue = 0, filteredPaid = 0, filteredUnpaid = 0;
        if (useDate) {
            const fParams = [];
            const fConds  = [];
            if (dateFrom) { fConds.push(`DATE(i.${safeField}) >= ?`); fParams.push(dateFrom); }
            if (dateTo)   { fConds.push(`DATE(i.${safeField}) <= ?`); fParams.push(dateTo); }
            const fWhere = fConds.length ? 'WHERE ' + fConds.join(' AND ') : '';
            const [[fr]] = await pool.query(
                `SELECT COALESCE(SUM(CASE WHEN status='paid' THEN amount ELSE 0 END),0) as rev,
                        SUM(CASE WHEN status='paid'   THEN 1 ELSE 0 END) as pd,
                        SUM(CASE WHEN status='unpaid' THEN 1 ELSE 0 END) as up
                 FROM invoices i ${fWhere}`, fParams
            );
            filteredRevenue = fr.rev;
            filteredPaid    = fr.pd;
            filteredUnpaid  = fr.up;
        }
