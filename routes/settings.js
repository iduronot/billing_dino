...
// POST - Test AI Agent (tanpa simpan ke history, tapi tetap sertakan konteks DB)
router.post('/api/test-ai-agent', async (req, res) => {
    try {
        const { testAIReply, invalidateSystemCache } = require('../helpers/ai-agent');
        ...
        // Invalidate cache agar pakai system prompt terbaru saat test
        invalidateSystemCache();

        const reply = await testAIReply(pool, api_key.trim(), model, system_prompt, test_message);
